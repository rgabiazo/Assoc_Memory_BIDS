...
log_file="${log_dir}/ses-endpoint_setup_topup_$(date +'%Y-%m-%d_%H-%M-%S').log"
...
for subject_folder in $(ls $source_dir); do
    if [[ $subject_folder == sub-* ]]; then
        ...
        func_source="$source_dir/$subject_folder/ses-endpoint/func"
        ...
        for bold_file in $(ls $func_source/*_bold.nii.gz); do
            ...
            run_func_directory="$destination_dir/$subject_folder/ses-endpoint/$formatted_run_number/func"
            ...
            bold_dest_name="${subject_folder}_ses-endpoint_acq-AP_${formatted_run_number}"
        ...
        for run_dir in $(ls $destination_dir/$subject_folder/ses-endpoint); do
            if [[ $run_dir == run-* ]]; then
                run_func_directory="$destination_dir/$subject_folder/ses-endpoint/$run_dir/func"
                rfMRI_PA_dest_name="${subject_folder}_ses-endpoint_acq-PA_${run_dir}"
