...
DEST_DIR="${BASE_DIR}/derivatives/ICA_AROMA/Assoc_Memory_1st-Level"
LOG_DIR="${BASE_DIR}/code/logs"
SCRIPT_DIR="${BASE_DIR}/code/scripts"

# Log file setup
LOG_FILE="${LOG_DIR}/ses-baseline_ICA_setup_$(date '+%Y-%m-%d_%H-%M-%S').log"

# Process each subject folder in the topup_correction directory for functional data
for sub_dir in ${TOPUP_DIR}/*; do
  ...
  if [ -d "${sub_dir}" ]; then
    ...
    for run in 01 02 03; do  # Adjust the number of runs as needed
      SRC_FUNC="${TOPUP_DIR}/${sub}/ses-baseline/run-${run}/func/topup_results_run_${run}/${sub}_corrected_task-assoc_memory_run-${run}_bold.nii.gz"
      
      if [ -f "${SRC_FUNC}" ]; then
        DEST_FUNC="${DEST_DIR}/${sub}/ses-baseline/func"
        mkdir -p "${DEST_FUNC}"
        ...
        CUSTOM_EVENTS_DIR="${BASE_DIR}/derivatives/ICA_AROMA/fsl_custom_events"
        mkdir -p "${CUSTOM_EVENTS_DIR}"

        # Process each events file and create txt files
        SES_PATH="${BIDS_DATA_DIR}/${sub}/ses-baseline/func"  # Adjusted to correct BIDS directory
        OUTPUT_SUB_DIR="${CUSTOM_EVENTS_DIR}/${sub}/ses-baseline"  # Custom output directory for txt files
        ...
        for events_file in $(ls $SES_PATH | grep 'task-assoc_memory.*events.tsv'); do
            ...
            for trial_type in encoding_pair recog_pair encoding_place recog_place encoding_face recog_face; do
                output_file="${OUTPUT_SUB_DIR}/${sub}_ses-baseline_task-assoc_memory_${trial_type}_${run_id}.txt"
    ...
    SRC_ANAT="${BIDS_DATA_DIR}/${sub}/ses-baseline/anat/${sub}_T1w.nii.gz"
    if [ -f "${SRC_ANAT}" ]; then
      DEST_ANAT="${DEST_DIR}/${sub}/ses-baseline/anat"
      mkdir -p "${DEST_ANAT}"
