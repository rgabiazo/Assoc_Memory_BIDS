...
process_and_combine_conditions() {
    ...
    output_dir="${BASE_DIR}/BIDS_dataset/$subject_id/ses-endpoint/func"
    
    combined_file=$(mktemp)
    tsv_file="$output_dir/${subject_id}_ses-endpoint_task-assoc_memory_run-$(printf "%02d" $run_num)_events.tsv"
