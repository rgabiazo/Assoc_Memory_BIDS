...
LOG_FILE="${LOG_DIR}/ses-baseline_blipupdown.sh_$(date '+%Y-%m-%d_%H-%M-%S').log"
...
process_run() {
...

    {
        ...
        # Directory paths
        RUN_DIR="${TOPUP_DIR}/${subject}/ses-baseline/run-${run}/func"
        ...
        # File for applytopup
        TASK_FILE="${RUN_DIR}/${subject}_ses-baseline_acq-AP_run-${run}.nii.gz"
