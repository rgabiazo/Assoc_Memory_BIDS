...
for subject in $(ls $DATASET_DIR | grep 'sub-'); do
    ...
    # Define paths
    ses_path="$DATASET_DIR/$subject/ses-baseline/func"
    anat_path="$DATASET_DIR/$subject/ses-baseline/anat"
    output_sub_dir="$output_dir/$subject/ses-baseline"
    fsl_anat_dir="$fsl_level_one_dir/$subject/ses-baseline/anat"
    fsl_func_dir="$fsl_level_one_dir/$subject/ses-baseline/func"
    ...
    for events_file in $(ls $ses_path | grep 'events.tsv'); do
        ...
        for trial_type in encoding_pair recog_pair encoding_place recog_place encoding_face recog_face; do
            output_file="${output_sub_dir}/${subject}_ses-baseline_task-assoc_memory_${trial_type}_${run_id}.txt"
