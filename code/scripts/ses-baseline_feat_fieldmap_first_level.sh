...
log_file="${log_dir}/ses-baseline_feat_fieldmap_first_level$(date +%Y-%m-%d-%H-%M-%S).log"
...
template_fsf="${BASE_DIR}/code/design_files/feat_fieldmap/Assoc_Memory/ses-baseline/Assoc_Memory_1st-Level/design.fsf"
...
for arg in "$@"; do
    ...
    for run in "${RUNS[@]}"; do
        ...
        sed -i '' -e "s|@func@|${data_base}/${sub}/ses-baseline/func/${sub}_corrected_task-assoc_memory_run-${run_pad}_bold|g" $output_fsf
