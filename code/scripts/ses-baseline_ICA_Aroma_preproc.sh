...
LOG_FILE="${LOG_DIR}/ses-baseline_ICA_Aroma_preproc.sh_$(date +'%Y-%m-%d_%H-%M-%S').log"
...
for arg in "$@"; do
    ...
    for RUN in "${RUN_ARRAY[@]}"; do
        ...
        OUTPUT_DIR="${AROMA_DIR}/${SUBJ_ID}/ses-baseline/feat_preproc/${SUBJ_ID}_corrected_task-assoc_memory_run-${RUN}_preproc"
        BOLD_FILE="${AROMA_DIR}/${SUBJ_ID}/ses-baseline/func/${SUBJ_ID}_corrected_task-assoc_memory_run-${RUN}_bold.nii.gz"
        HIGHRES_FILE="${AROMA_DIR}/${SUBJ_ID}/ses-baseline/anat/${SUBJ_ID}_T1w_brain.nii.gz"
        ...
        BET_OUTPUT="${AROMA_DIR}/${SUBJ_ID}/ses-baseline/func/${SUBJ_ID}_ICA_Aroma_task-Assoc_memory_run-${RUN}"
